...
REQUIRED_FUNCS_DOC = (
    ...
    "(feature engineering; applied identically at train and inference time)\n"
    "build_model(config: dict | None) -> Any  (return an untrained model object)\n"
    "train(model, X_processed, y: np.ndarray[N,128,128]) -> Any  "
    "(return the trained model; must have a non-empty docstring)\n"
    "The returned model must implement .predict(X_processed) -> np.ndarray[N,128,128] "
    "with binary {0,1} values."
)
